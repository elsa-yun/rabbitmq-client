 ...
 *
 */
public class CopyOfMqProductConnectionSingleton {

	private static final Log logger = LogFactory.getLog(CopyOfMqProductConnectionSingleton.class);

	private static class ConnectionHolder {
		private static final Map<Long, Connection> holder = new HashMap<Long, Connection>();
	}

	private static class ConnectionCntHolder {
//		private static final int connectionCount = MqBaseConfig.getBaseConfig().getProductConnectionCount();
	...
	private static class ExecutorServiceHolder {
		...
		static {
//			addrArr = getAddressArray(MqBaseConfig.getBaseConfig().getHosts());
			addrArr = getAddressArray("172.16.100.43:5672,172.16.100.44:5672");
			// int threadCount =
			// MqBaseConfig.getBaseConfig().getProductThreadCount();
...
//			factory.setAutomaticRecoveryEnabled(MqBaseConfig.getBaseConfig().isAutomaticRecoveryEnabled());
		...
		// }
		// } catch (Exception e) {
		// logger.error("MqProductConnectionSingleton shutDownPool exception for=>"
		// + e.toString());
		// }
		// }
	...
	public static Connection connection() {
		...
		if (!isOk(ConnectionHolder.holder.get(curr))) {
			// synchronized (CopyOfMqProductConnectionSingleton.class) {
			if (!isOk(ConnectionHolder.holder.get(curr))) {
				destory(curr);
	...
	public static void destoryall() {
		for (Map.Entry<Long, Connection> entry : ConnectionHolder.holder.entrySet()) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(("mqproduct connection entry key is " + entry.getKey() + " is close"));
			}
		}
	}
	...
	private CopyOfMqProductConnectionSingleton() {
